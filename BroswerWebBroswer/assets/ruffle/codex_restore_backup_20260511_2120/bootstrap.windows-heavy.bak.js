(function () {
    ...
    function buildSurfaceSnapshot(host, player) {
        ...
        var canvas = null;
        try {
            canvas = player && player.shadowRoot ? player.shadowRoot.querySelector("canvas") : null;
        } catch (error) {
            canvas = null;
        }
        ...
        return {
            ...
            visualViewportScale: window.visualViewport && window.visualViewport.scale
                ...
                : 1,
            host: describeBox(host),
            player: describeBox(player),
            canvas: canvas ? {
    ...
    function installPlayerDiagnostics(player, host, originalUrl, proxyUrl) {
        if (!player || player.__pvzolDiagnosticsInstalled) {
            return;
        }

        player.__pvzolDiagnosticsInstalled = true;
        var lastSnapshot = "";

        function emit(reason) {
            var snapshot = buildSurfaceSnapshot(host, player);
            var encoded = stringifyForLog(snapshot);
            if (encoded === lastSnapshot) {
                return;
            }
            lastSnapshot = encoded;
            logInfo("player surface", {
                reason: reason,
        ...
        emit("created");
        window.setTimeout(function () { emit("after-0ms"); }, 0);
        window.setTimeout(function () { emit("after-250ms"); }, 250);
        window.setTimeout(function () { emit("after-1000ms"); }, 1000);
        window.setTimeout(function () { emit("after-3000ms"); }, 3000);

        if (window.ResizeObserver) {
            var observer = new ResizeObserver(function () {
                emit("resize-observer");
            });
            try {
                if (host) {
                ...
                logError("resize observer install failed", String(error));
            }
        }

        if (player.shadowRoot && window.MutationObserver) {
            var mutationObserver = new MutationObserver(function () {
                emit("shadow-mutation");
            });
            try {
                mutationObserver.observe(player.shadowRoot, {
                    childList: true,
                    subtree: true,
                    attributes: true
                });
            } catch (error) {
                logError("shadow observer install failed", String(error));
            }
        }
    }

    function attachDiagnosticsToActivePlayers(reason) {
        ...
        for (var i = 0; i < players.length; i += 1) {
            ...
            var proxyUrl = "";
            try {
                originalUrl = player.getAttribute("data-pvzol-flash-url") || "";
                proxyUrl = player.getAttribute("data-pvzol-flash-proxy-url") || originalUrl;
            } catch (error) {
                originalUrl = "";
                proxyUrl = "";
            }
            installPlayerDiagnostics(player, player, originalUrl, proxyUrl);
        }
        logInfo("active player diagnostics scan", {
            reason: reason,
    ...
    function injectCompatibilityStyle() {
        ...
        style.textContent =
            ...
            "}" +
            ".pvzol-ruffle-host>ruffle-player,.pvzol-ruffle-host>ruffle-embed,.pvzol-ruffle-host>ruffle-object{" +
            "display:block !important;" +
            "margin:0 !important;" +
    ...
    function normalizeFlashLayout(root) {
        ...
        for (var i = 0; i < elements.length; i += 1) {
            ...
            if (width > 0 && height > 0) {
                var lockedWidth = String(Math.round(width)) + "px";
                var lockedHeight = String(Math.round(height)) + "px";
                element.style.width = lockedWidth;
                element.style.minWidth = lockedWidth;
                element.style.maxWidth = lockedWidth;
                element.style.height = lockedHeight;
                element.style.minHeight = lockedHeight;
                element.style.maxHeight = lockedHeight;
                element.style.aspectRatio = String(width) + " / " + String(height);
            ...
            if (width > 0) {
                var widthOnly = String(Math.round(width)) + "px";
                element.style.width = widthOnly;
                element.style.minWidth = widthOnly;
                element.style.maxWidth = widthOnly;
            }

            if (height > 0) {
                var heightOnly = String(Math.round(height)) + "px";
                element.style.height = heightOnly;
                element.style.minHeight = heightOnly;
                element.style.maxHeight = heightOnly;
            }
        }
    }
    ...
    function pixelDimensionValue(rawValue, fallbackPixels) {
        if (rawValue) {
            ...
            var pixelMatch = /^(\d+(?:\.\d+)?)px$/i.exec(text);
            if (pixelMatch) {
                return String(Math.round(parseFloat(pixelMatch[1]))) + "px";
            }
        }
        ...
        if (typeof window.fetch === "function") {
            ...
            window.fetch = function (input, init) {
                try {
                    ...
                    var rewrittenFetchUrl = "";
                    if (typeof input === "string") {
                        originalFetchUrl = input;
                        rewrittenFetchUrl = toProxyUrl(input);
                        if (rewrittenFetchUrl && rewrittenFetchUrl !== input) {
                            input = rewrittenFetchUrl;
                        }
                    } else if (input && typeof input.url === "string") {
                        originalFetchUrl = input.url;
                        rewrittenFetchUrl = toProxyUrl(input.url);
                        if (rewrittenFetchUrl
                            && rewrittenFetchUrl !== input.url
                            && (!input.method || input.method.toUpperCase() === "GET" || input.method.toUpperCase() === "HEAD")) {
                            input = new Request(rewrittenFetchUrl, input);
                        } else if (rewrittenFetchUrl && rewrittenFetchUrl !== input.url) {
                            logInfo("fetch rewrite skipped for body-bearing request", {
                                originalUrl: originalFetchUrl,
                                effectiveUrl: input.url,
                                requestedUrl: rewrittenFetchUrl,
    ...
    function createReplacementHost(element, originalUrl, proxyUrl) {
        ...
        if (rawStyle) {
            host.setAttribute("style", rawStyle + ";display:block;box-sizing:border-box;overflow:hidden;position:relative;contain:layout paint size;");
        }
        ...
        var lockedWidth = pixelDimensionValue(element.getAttribute("width"), element.clientWidth);
        var lockedHeight = pixelDimensionValue(element.getAttribute("height"), element.clientHeight);
        if (lockedWidth) {
            host.setAttribute("data-pvzol-locked-width", lockedWidth);
        }
        if (lockedHeight) {
            host.setAttribute("data-pvzol-locked-height", lockedHeight);
        }
        if (width) {
            host.style.width = lockedWidth || width;
            host.style.minWidth = lockedWidth || width;
            host.style.maxWidth = lockedWidth || width;
        }
        if (height) {
            host.style.height = lockedHeight || height;
            host.style.minHeight = lockedHeight || height;
            host.style.maxHeight = lockedHeight || height;
        } else if (element.clientHeight > 0) {
            var fallbackHeight = String(Math.round(element.clientHeight)) + "px";
            host.style.height = fallbackHeight;
            host.style.minHeight = fallbackHeight;
            host.style.maxHeight = fallbackHeight;
        }
        ...
        if (lockedWidth && lockedHeight) {
            host.style.aspectRatio = lockedWidth.replace("px", "") + " / " + lockedHeight.replace("px", "");
        }
        return host;
    }
    ...
    function replaceFlashElementWithPlayer(element, reason) {
        ...
        try {
            ...
            var lockedPlayerWidth = host.getAttribute("data-pvzol-locked-width") || "";
            var lockedPlayerHeight = host.getAttribute("data-pvzol-locked-height") || "";
            player.style.width = lockedPlayerWidth || "100%";
            player.style.height = lockedPlayerHeight || "100%";
            player.style.display = "block";
            player.style.minWidth = lockedPlayerWidth || "";
            player.style.maxWidth = lockedPlayerWidth || "100%";
            player.style.minHeight = lockedPlayerHeight || "";
            player.style.maxHeight = lockedPlayerHeight || "100%";
            ...
            player.addEventListener("loadedmetadata", function () {
                ...
                installPlayerDiagnostics(player, host, originalUrl, proxyUrl);
            });
    ...
    function onRuffleReady() {
        ...
        window.requestAnimationFrame(function () {
            attachDiagnosticsToActivePlayers("ruffle-ready-raf");
        });
        window.setTimeout(function () {
            attachDiagnosticsToActivePlayers("ruffle-ready-0ms");
        }, 0);
        window.setTimeout(function () {
            scheduleFlashRefresh("ruffle-ready-delayed");
        }, 200);
        window.setTimeout(function () {
            attachDiagnosticsToActivePlayers("ruffle-ready-250ms");
        }, 250);
        window.setTimeout(function () {
            attachDiagnosticsToActivePlayers("ruffle-ready-1000ms");
        }, 1000);
    }
    ...
    window.addEventListener("resize", function () {
        normalizeFlashLayout(document);
        attachDiagnosticsToActivePlayers("window-resize");
    });
